...
func NewPxCanvas(state *apptype.State, config apptype.PxCanvasConfig) *PxCanvas {
	canvas := &PxCanvas{PxCanvasConfig: config,
		appState: state}
...
func (pxCanvas *PxCanvas) MouseToCanvasXY(ev *desktop.MouseEvent) (*int, *int) {
	...
	xOffsett := pxCanvas.CanvasOffset.X
	yOffsett := pxCanvas.CanvasOffset.Y

	x := int((ev.Position.X - xOffsett) / pxSize)
	y := int((ev.Position.Y - yOffsett) / pxSize)
...
func (pxCanvas *PxCanvas) NewDrawing(cols, rows int) {
	...
	pixelData := NewBlankImage(cols, rows, color.NRGBA{
		R: 128,
		G: 128,
		B: 128,
		A: 255,
